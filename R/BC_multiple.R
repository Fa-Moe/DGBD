...
BC_multiple <- function(df_abundance, columns_select_multiple=NULL,is_phyloseq=FALSE,supress_messages=TRUE,...){
  ...
  for (i in 1:sample_amount){
    ...
    temp_row <- c(I(list(temp_plot)),I(list(temp_report)))
  ...
  colnames(multiple_analysis) <- c("otput_BC_plots","output_BC_reports")
