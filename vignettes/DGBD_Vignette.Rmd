...
```
This package includes functions to assist in conducting a step-by-step DGBD modeling process. The guide will walk you through each stage, explaining the purpose of each function and demonstrating how to customize the analysis according to your specific needs.

The `BC_rank` function imports frequency data from a specified data frame in your workspace and ranks it. You need to specify the `column_rank` parameter, which indicates the column containing the data you want to analyze. Once the ranking process is complete, the column is automatically labeled as **abundance** for smooth processing in subsequent stages.
```{r}
library(DGBD)
...
The `BC_param` function imports results from `BC_rank` and incorporates them into the model formula. It creates two new columns in the data frame, denoted as `pre_numerator` and `pre_denominator.` These columns undergo log transformation and are then used to fit a linear model, estimating the parameters 'A,' 'a', and 'b.' Subsequently, the parameters are linearized and made ready for reporting.
