...
The functions included in this package perform a DGBD modeling step by step. This Vignette follows the process along to explain what each function does, and how you can customize the analysis.

BC_rank loads the frequency data from a data frame in the environment and ranks it. It has to be given a "column_rank" argument that points to the column we want to analyze.
BC_rank automatically labels the used column as "abundance" for further processing.
...
BC_param automatically imports the results from BC_rank and then fits them into the formula. New columns are added to the data frame with "pre_numerator" and "pre_denominator" names. These are then log() transformed and fitted to a linear model to approximate the 'A', 'a' and 'b' parameters. The parameters are then linearized for report. 
